...
namespace OKEX.Auto.Core.Domain.Manager.OKEX
{
    ...
    /// okex 接口 http 请求拦截器
    /// </summary>
    public class OkexHttpInterceptor : DelegatingHandler
    {
        private string _apiKey;
        private string _passPhrase;
        private string _secret;
        private string _bodyStr;
        public OkexHttpInterceptor(string apiKey, string secret, string passPhrase, string bodyStr)
        {
            this._apiKey = apiKey;
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                sign = EncryptionUtility.HmacSHA256($"{timeStamp}{method}{requestUrl}{this._bodyStr}", this._secret);
            }
            else
            {
                sign = EncryptionUtility.HmacSHA256($"{timeStamp}{method}{requestUrl}", this._secret);
            }
